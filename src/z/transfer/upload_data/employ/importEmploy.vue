...
<script>
  ...
  export default {
    ...
    data() {
      return {
        importAction: process.env.BASE_API + '/api/employ/import_employ_excel',
        show: false,
        importFileList: [],
        importHeaders: {'X-Token': getToken()},
        importStatus: {totalRowCount: 0, nowRowCount: 0, status: -2},
        importUseTime: {start: new Date(), useTime: 0},
        downloadTemplateUrl: '/static/templates/employ/employ.xlsx',
        uploadData: {
        }
      }
    },
    ...
    methods: {
      ...
      uploadStatus() {
        getStatus({key: 'data_employ_info'}).then(response => {
