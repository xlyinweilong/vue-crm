...
<script>
  ...
  export default {
    ...
    data() {
      return {
        importAction: process.env.BASE_API + '/api/erp_goods/import_goods_excel',
        show: false,
        importFileList: [],
        importHeaders: {'X-Token': getToken()},
        importStatus: {totalRowCount: 0, nowRowCount: 0, status: -2},
        importUseTime: {start: new Date(), useTime: 0},
        downloadTemplateUrl: '/static/templates/goods/goods.xlsx',
        uploadData: {
        }
      }
    },
    ...
    methods: {
      ...
      uploadStatus() {
        getStatus({key: 'data_goods_info'}).then(response => {
