...
<script>
  ...
  export default {
    ...
    data() {
      return {
        importAction: process.env.BASE_API + '/api/shop/goods/import_stock_excel',
        show: false,
        importFileList: [],
        importHeaders: {'X-Token': getToken()},
        importStatus: {totalRowCount: 0, nowRowCount: 0, status: -2},
        importUseTime: {start: new Date(), useTime: 0},
        downloadTemplateUrl: '/static/templates/shop/goods/stock.xlsx',
        uploadData: {
          onlyOnShelf: '0'
        }
      }
    ...
    methods: {
      ...
      uploadStatus() {
        getStatus({key: 'goods_stock'}).then(response => {
