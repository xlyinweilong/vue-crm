...
<script>
  ...
  export default {
    ...
    data() {
      return {
        importAction: process.env.BASE_API + '/api/shop/goods_list_detail/import_excel',
        show: false,
        importFileList: [],
        importHeaders: {'X-Token': getToken()},
        importStatus: {totalRowCount: 0, nowRowCount: 0, status: -2},
        importUseTime: {start: new Date(), useTime: 0},
        downloadTemplateUrl: '/static/templates/shop/goodsList/goods_list.xlsx',
        uploadData: {
          shopGoodsListId: ''
        }
      }
    ...
    methods: {
      ...
      uploadStatus() {
        getStatus({key: 'shop_goods_list'}).then(response => {
      ...
      onOpen(shopGoodsListId) {
        this.uploadData.shopGoodsListId = shopGoodsListId
        this.importFileList = []
