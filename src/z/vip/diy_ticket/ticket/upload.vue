...
<script>
  ...
  export default {
    ...
    data() {
      return {
        importAction: process.env.BASE_API + '/api/vip/ticket_goods/import_excel',
        show: false,
        importFileList: [],
        importHeaders: {'X-Token': getToken()},
        importStatus: {totalRowCount: 0, nowRowCount: 0, status: -2},
        importUseTime: {start: new Date(), useTime: 0},
        downloadTemplateUrl: '/static/templates/ticket/ticket_goods.xlsx',
        uploadData: {
          ticketId: ''
        }
      }
    ...
    methods: {
      ...
      uploadStatus() {
        getStatus({key: 'ticket_goods'}).then(response => {
      ...
      onOpen(ticketId) {
        this.uploadData.ticketId = ticketId
        this.importFileList = []
